...
os.environ["OPENAI_API_KEY"] = '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'
...
def load_index_from_firebase():
    ...
    for blob in blobs:
        blob_name = blob.name
        if blob_name.endswith('.json'):  # Process only JSON files
            local_filename = os.path.join(local_directory, os.path.basename(blob_name))
...
st.info("Welcome to the Knowt Chatbot. This chatbot takes files you've uploaded and answers questions about them. You can also use it to generate questions to aid in your studying. To begin, visit the file explorer page through the sidebar for instruction on how to upload files. Use /generatequestions to generate a list of 10 questions based on your documents, and use /generatecsv to generate a CSV vocab list that can be imported into programs like Anki and Quizlet")

st.info("If you've added or removed any files, please press the button below so I can refresh my memory!")
